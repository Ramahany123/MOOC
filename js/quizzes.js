...
function saveQuizResult(quizId, scorePercentage, correctCount, totalCount) {
    // Check if the global user ID is set (meaning the user is authenticated)
    if (!window.currentUserId || !window.db) {
        // --- ADDED ENHANCED DEBUGGING ---
        console.error("Cannot save quiz result: User is not authenticated or Firebase DB is not initialized.", {
            userId: window.currentUserId,
            dbInitialized: !!window.db // Will log true or false
        });
        // ---------------------------------
        return;
    }
    ...
    const quizDataEntry = { // Renamed local variable in my mind, but kept original for stability
    ...
    .then(() => {
        console.log(`Quiz ${quizId} result saved successfully! Score: ${scorePercentage}%`);
    })
    .catch(error => {
